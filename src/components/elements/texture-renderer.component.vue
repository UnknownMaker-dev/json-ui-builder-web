...
const drawTexture = async () => {
  if (!canvasRef.value || !props.src || props.width <= 0 || props.height <= 0) return
  ...
  const newImageData = new ImageData(newPixels, props.width, props.height)
  ctx.putImageData(newImageData, 0, 0)
}

// Redesenha sempre que as propriedades mudarem
watch(() => [props.src, props.nineslice, props.width, props.height], drawTexture, { deep: true })
